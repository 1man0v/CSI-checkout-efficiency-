...
const I18N = {
  ru: {
    ...
    "kpi.hourly_load": "Загрузка кассовой линии", "kpi.hourly_load_sub": "спрос POS+КСО по часам работы магазина vs. пропускная способность касс",
    "chart.hourly_load_no_overload": "Избытка открытых POS в выбранном периоде не выявлено.",
    "chart.legend_pos": "Чеки POS", "chart.legend_sco": "Чеки КСО",
    "chart.legend_sco_capacity": "Макс. пропускная способность КСО", "chart.legend_pos_capacity": "Макс. пропускная способность POS",
    "chart.legend_excess": "Избыток открытых POS",
    "chart.hourly_load_single_day": "Данные за {date} (выбран конкретный день)",
    "chart.hourly_load_avg_range": "Среднее по часу дня за период {from}–{to}",
    ...
    "diagnosis.stale_hint": "Касса {id} не передает телеметрию {days} дн. — фактически не в эксплуатации, хотя числится в реестре кассового сервера. Вероятная причина низкой доступности магазина.",
    "diagnosis.stale_task_title": "{id}: выяснить причину отсутствия телеметрии ({days} дн.)",
    "diagnosis.stale_task_kpi": "Восстановить телеметрию/эксплуатацию кассы {id}",
    ...
    "diagnosis.business_potential_hint": "Доля чеков КСО {share}% при нормативе 30%. Целевой клиент КСО на POS: потенциал перевода {potential}% от нагрузки POS ({load} чек/нед) — покупатели с небольшим чеком и оплатой картой стоят в очереди к кассиру, хотя могли бы обслужить себя сами. Одного устранения точечных технических причин (ниже, если есть) для выхода на норматив недостаточно — нужно системно перераспределять поток.",
    ...
    "diagnosis.utilization_excess_hint": "Высокая суммарная недельная нагрузка POS — повод считать магазин аутсайдером, но по часовому анализу пропускной способности касс нехватки открытых POS не выявлено: почти весь день ({hours}) касс КСО достаточно, чтобы принять остаток спроса после открытых POS. Настоящая причина упущенной эффективности здесь — не дефицит касс, а избыточное количество открытых POS при доступной мощности КСО.",
    "diagnosis.utilization_excess_task_title": "Сократить количество открытых POS, перенаправить поток на КСО",
    "diagnosis.utilization_excess_task_kpi": "Открытых POS не больше расчетной потребности по часам",
  ...
  en: {
    ...
    "kpi.hourly_load": "Register line load", "kpi.hourly_load_sub": "POS+SCO demand by hour vs. register throughput capacity",
    "chart.hourly_load_no_overload": "No excess open POS lanes detected in the selected period.",
    "chart.legend_pos": "POS checks", "chart.legend_sco": "SCO checks",
    "chart.legend_sco_capacity": "Max. SCO throughput", "chart.legend_pos_capacity": "Max. POS throughput",
    "chart.legend_excess": "Excess open POS lanes",
    "chart.hourly_load_single_day": "Data for {date} (specific day selected)",
    "chart.hourly_load_avg_range": "Hourly average over {from}–{to}",
    "currency.RUB": "₽", "currency.USD": "$", "currency.EUR": "€",
    ...
    "diagnosis.stale_hint": "Register {id} hasn't reported telemetry for {days} days — effectively out of service, though still listed in the register server. Likely cause of the store's low availability.",
    "diagnosis.stale_task_title": "{id}: find out why telemetry stopped ({days}d)",
    "diagnosis.stale_task_kpi": "Restore telemetry/operation of register {id}",
    ...
    "diagnosis.business_potential_hint": "SCO share is {share}% against a 30% norm. Target SCO customer on POS: transfer potential is {potential}% of POS load ({load} checks/week) — shoppers with small baskets and card payment are queuing for a cashier when they could check themselves out. Fixing point technical causes alone (below, if any) won't be enough to reach the norm — the flow needs to be rebalanced systemically.",
    ...
    "diagnosis.utilization_excess_hint": "Total weekly POS load is high enough to flag this store as an outlier, but the hourly capacity analysis finds no shortage of open POS lanes: for most of the day ({hours}) the SCO terminals alone could absorb the residual demand after open POS. The real cause of lost efficiency here isn't a register shortage — it's too many open POS lanes given the available SCO capacity.",
    "diagnosis.utilization_excess_task_title": "Reduce open POS lanes, redirect flow to SCO",
    "diagnosis.utilization_excess_task_kpi": "Open POS lanes no higher than the hourly requirement",
